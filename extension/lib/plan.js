...
(function () {
  ...
  function buildTerminologySystemPrompt() {
    return [
      ...
      'Mặc định là action="translate". Bản lồng tiếng để nghe: câu lẫn quá nhiều từ tiếng Anh thì rất khó nghe.',
      'Chỉ dùng action="keep" cho tên riêng, tên sản phẩm, định danh trong code, viết tắt,',
      'và những thuật ngữ lõi mà giảng viên Việt gần như luôn nói bằng tiếng Anh',
      '(ví dụ machine learning, gradient descent, overfitting, framework).',
      'Không được dịch đen nhãn kỹ thuật mang tính ẩn dụ hay tên pattern — thà giữ tiếng Anh còn hơn dịch sai nghĩa.',
      'Còn lại, nếu tiếng Việt có cách gọi thông dụng thì dùng: loss function -> hàm mất mát, layer -> lớp, training -> huấn luyện.',
      '',
  ...
  function buildTerminologyRetryUserPrompt(segments) {
    ...
    return [
      ...
      'Với mỗi thuật ngữ, dùng action="translate" nếu tiếng Việt có cách gọi thông dụng; chỉ keep khi ngành luôn nói bằng tiếng Anh.',
  ...
  function buildTranslateSystemPrompt(rate, terminology) {
    ...
    return [
      ...
      '- Dịch sang tiếng Việt mọi thứ có cách gọi thông dụng. Đây là bản để NGHE: một câu lẫn nhiều từ tiếng Anh nghe rất mệt.',
      '- Chỉ giữ tiếng Anh cho tên riêng, tên sản phẩm, định danh code, viết tắt, và thuật ngữ lõi mà giảng viên Việt gần như luôn nói bằng tiếng Anh.',
      '- Cố gắng không quá hai cụm tiếng Anh trong một câu. Nếu câu đang lẫn nhiều hơn thế, dịch bớt những cụm có cách gọi Việt rõ ràng.',
      '- Không dịch đen nhãn kỹ thuật mang tính ẩn dụ, idiom hay tên pattern: chỗ nào dịch ra sai nghĩa thì giữ nguyên tiếng Anh.',
  ...
  function buildTerminologyReviewSystemPrompt() {
    return [
      ...
      'Ngược lại, mục nào có cách gọi Việt thông dụng mà đang để action="keep" thì đổi sang "translate" — bản lồng tiếng lẫn quá nhiều tiếng Anh rất khó nghe.',
